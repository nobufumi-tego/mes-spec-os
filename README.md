...
> ⚠️ **本リポジトリは構想段階（v0.2.6）です。** 実運用には自社環境での検証が必要であり、出力は **「下書きのたたき台」** として扱ってください。
>
...
- Claude Code（インストール手順は [docs/インストール.md](docs/インストール.md) を参照）
...
1. このフォルダに `cd` する
2. clone 直後なら `git remote remove origin` を実行（ローカル運用前提のため。詳細は [docs/インストール.md](docs/インストール.md)）
3. `reference/ENAA/README.md` の手順で ENAA 5 資料をダウンロード・配置（5 分程度）
4. PowerShell で `claude` を起動
5. 起動後、「**こんにちは。**」と打って Enter（最後の句点まで打つのがコツ）
6. メニューから `1) MES 仕様書を作る` を選ぶ
7. 案件情報・現状・要件を 1 問 1 答で入力
8. `outputs/MES仕様書/<日付>_<案件名>.yaml` と `.md` が生成される
...
| `/保存` | 作業の節目にローカル Git へ保存する（機密の混入チェック付き） |
...
| `outputs/` | 生成された仕様書（git 追跡はサンプルのみ。実生成資料は `.gitignore` で除外） |
| `doctor.ps1` | 環境診断スクリプト（必要ツール・ENAA 配置・リモート設定をチェック） |
| `tests/` | cache.py の単体テスト（ENAA 実ファイル不要）＋ ENAA 配置時のみ動く統合テスト |
| `CHANGELOG.md` | 変更履歴 |
...
- **5 分で試す（チュートリアル）** → [docs/5分で試す.md](docs/5分で試す.md)
- **インストール手順（詳しい版）** → [docs/インストール.md](docs/インストール.md)
- **困ったとき** → [docs/困ったとき.md](docs/困ったとき.md)

### 利用前必読

- **セキュリティと情報漏洩対策** → [docs/セキュリティ.md](docs/セキュリティ.md)
- **サポート方針（個別対応なし）** → [SUPPORT.md](SUPPORT.md)
...
- 抽出レシピ集 → [usecases/MES仕様書/extraction-recipes.md](usecases/MES仕様書/extraction-recipes.md)
